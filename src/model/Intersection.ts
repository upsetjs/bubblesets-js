...
export class Intersection {
  ...
  static intersectLineLine(la: Line, lb: Line) {
  ...
  static fractionAlongLineA(la: Line, lb: Line) {
    const uaT = (lb.x2 - lb.x1) * (la.y1 - lb.y1) - (lb.y2 - lb.y1) * (la.x1 - lb.x1);
    const ubT = (la.x2 - la.x1) * (la.y1 - lb.y1) - (la.y2 - la.y1) * (la.x1 - lb.x1);
    const uB = (lb.y2 - lb.y1) * (la.x2 - la.x1) - (lb.x2 - lb.x1) * (la.y2 - la.y1);
    if (uB) {
  ...
  static fractionToLineEnd(bounds: Rectangle, line: Line) {
    let minDistance = Number.POSITIVE_INFINITY;
    let countIntersections = 0;
  ...
  static testIntersection(line: Line, bounds: Rectangle, intersections: Intersection[]) {
    let countIntersections = 0;
